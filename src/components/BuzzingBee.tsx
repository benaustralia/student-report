...
export const BuzzingBee: React.FC<BuzzingBeeProps> = ({ className = '' }) => {
  ...
  useEffect(() => {
    ...
    // Scroll-triggered buzzing around happily - more subtle movements
    gsap.to(bee, {
      x: () => window.innerWidth * 0.15, // Smaller movement
    ...
    gsap.to(bee, {
      x: () => -window.innerWidth * 0.1, // Smaller movement
    ...
    gsap.to(bee, {
      x: 0,
      y: () => window.innerHeight * 0.15, // Smaller movement
      scrollTrigger: {
        trigger: "body",
        start: "75% top",
        end: "bottom bottom",
    ...
    // Additional vertical bobbing - more subtle
    gsap.to(bee, {
  ...
  return (
    <div 
      ref={beeContainerRef}
      className={`fixed top-[30%] left-[10%] w-[100px] h-[100px] z-[100] pointer-events-none ${className}`}
    >
      <svg 
        id="bee" 
        viewBox="-5 -2 41.4 22.02" 
        xmlns="http://www.w3.org/2000/svg"
        className="w-full h-auto drop-shadow-[2px_2px_4px_rgba(0,0,0,0.2)] overflow-visible"
      >
        <path 
          ref={rightWingRef}
          id="right_wing" 
          className="fill-[#eec93a]" 
