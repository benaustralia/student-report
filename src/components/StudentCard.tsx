...
import { generateBilingualReport } from '@/services/aiReportService';
...
export const StudentCard = memo(function StudentCard({ student, classData, isSelected, onStudentSelected }: StudentCardProps) {
  ...
  const generateAIReport = async () => {
    ...
    try {
      const text = await generateBilingualReport({
        student,
        classData,
        notes: state.reportText,
        apiKey: import.meta.env.VITE_OPENAI_API_KEY
      });
      if (text) setState(prev => ({ ...prev, reportText: text, hasUnsavedChanges: true }));
      else alert('Failed to generate report text. Please try again.');
    } catch (error) {
  ...
  useAutoSaveReport({
    ...
    setHasUnsaved: (v) => setState(prev => ({ ...prev, hasUnsavedChanges: v })),
    saveReport
  });
